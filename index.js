...
async function starts() {
	...
	client.on('group-participants-update', async (anu) => {
		...
		try {
			...
			if (anu.action == 'add') {
				...
				teks = `Flw, vai pela sombra pq bosta no sol seca @${num.split('@')[0]} vai nem fazer falta👋`
				let buff = await getBuffer(ppimg)
				client.sendMessage(mdata.id, buff, MessageType.image, {caption: teks, contextInfo: {"mentionedJid": [num]}})
			}
		} catch (e) {
	...
	client.on('chat-update', async (mek) => {
		try {
			...
			const ownerNumber = ["558981246187@s.whatsapp.net"] // replace this with your number
			...
			switch(command) {
				...
				case 'meme':
					...
					break
                case 'Oi':
		        if (args.length < 1) return reply ('Oi meu chapa')
		            break
                case 'Td bem?':
		        if (args.length < 1) return reply ('Tudo, e tu parça? ')
		            break
                case 'Alguem aí pra jogar?':
		        if (args.length < 1) return reply ('Tem eu mais eu sou apenas um bot 😔')
		            break
                case 'Te amo':
		        if (args.length < 1) return reply ('Tbm te amo❤️')
		            break
                case 'bot top':
		        if (args.length < 1) return reply ('sou o mais top daqui irmão')
		            break
                case 'Bom dia':
		        if (args.length < 1) return reply ('Bom dia meu chapa☀️')
		            break
                case 'Boooora':
		        if (args.length < 1) return reply ('Calma ae vey')
		            break
                case 'Porra':
		        if (args.length < 1) return reply ('Sem treta vey')
		            break
                case 'Raí':
		        if (args.length < 1) return reply ('Meu dono provavelmente está ocupado\n então não ligue😡')
		            break
